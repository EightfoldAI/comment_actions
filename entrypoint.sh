...
if [[ $comment_body =~ ^needs_sandbox(:${alternate_python_version})(:(eu|gov|ca|uae|wu))?(:(dev|([0-9]+)(\.([0-9]+)?)?))?(:(([a-zA-Z0-9,]+)))?\s*?$ ]]; then
  for label in $labels; do
    case $label in
...
elif [[ $comment_body =~ ^needs_sandbox(:(eu|gov|ca|uae|wu))?(:(dev|([0-9]+)(\.([0-9]+)?)?))?(:(([a-zA-Z0-9,]+)))?\s*?$ ]]; then
  for label in $labels; do
    case $label in
      sandbox)
        already_needs_sandbox=true
